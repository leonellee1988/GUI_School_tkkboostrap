...
window.title('Ingreso datos materia')
...
#Name field:
label_config('Materia', ('Samanata', 11), 10, 20)
entry_name = ttkbootstrap.Entry(window, bootstyle='primary')
entry_name.place(x=100, y=20)

#Mode field:
label_config('Modalidad', ('Samanata', 11), 10, 60)
mode = ['PRESENCIAL', 'VIRTUAL', 'HIBRIDO']
combobox_mode = ttkbootstrap.Combobox(window,values=mode, bootstyle='primary')
combobox_mode.place(x=100, y=60)

#Credits field:
label_config('Créditos', ('Samanata', 11), 10, 100)
credit = [1, 2, 3, 4, 5]
combobox_credit = ttkbootstrap.Combobox(window, values=credit, bootstyle='primary')
combobox_credit.place(x=100, y=100)

#Cost field:
label_config('Costo', ('Samanata', 11), 10, 140)
entry_cost = ttkbootstrap.Entry(window, bootstyle='primary')
entry_cost.place(x=100, y=140)

#Career code:
label_config('Código carrera', ('Samanata', 11), 10, 180)
...
combobox_code_career.place(x=140, y=180)

#Load button:
load_button = ttkbootstrap.Button(text='Subir', command=load_data, bootstyle='success').place(x=90, y=250)

#Career button:
career_button = ttkbootstrap.Button(text='Carrera', command=career_check, bootstyle='warning').place(x=190, y=250)
